...
export function Card({course, taskDescription, daysLeft, textColor}){
    ...
    return(
            <div className="swiper-slide bg-white text-gray-950 font-semibold py-6 px-5 rounded-2xl mt-10 h-60">
                <h2 className="md:text-xl text-lg pb-6 2xl:pb-9">{course}</h2>
                <p className="xl:text-sm 2xl:text-base">{taskDescription}</p>
                <p className={`text-xl font-bold xl:my-2 2xl:my-6 my-6 ${textColor}`}>{daysLeft}</p>
            </div> 
    )  
}
